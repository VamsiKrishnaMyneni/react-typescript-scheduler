...
function Scheduler(props: SchedulerProps) {
  ...
  const getMonthRows = () => {
    ...
    let startOnSunday = (
      ...
      'Sun'.toUpperCase() === weekDays[0].toUpperCase()
    )
  ...
  const getWeekRows = () => {
    ...
    for (let i = 0; i <= HOURS; i++) {
      ...
      if (i > 0) {
        ...
        let obj: { id: string; label: string; days: { id: string; date: Date; data: any[] }[] } = { id: id, label: label, days: [] }
   ...
   * @return void
   */
  const handleDateChange = (daysInMonth: number, selectedDate: Date) => {
    ...
    <Paper variant="outlined" elevation={0} sx={{ p: 0 }}>
      <DateFnsLocaleContext.Provider value={dateFnsLocale}>
        <SchedulerToolbar
          ...
          onAlertCloseButtonClicked={onAlertCloseButtonClicked} locale={'en-US'} />
        <Grid
          container
          spacing={0}
          alignItems="center"
            ...
            <TransitionMode in>
              <Grid item xs={12}>
                <MonthModeView
                  ...
                  onCellClick={(event, row, day) => onCellClick && onCellClick(day.date)}
            ...
            <Grid container spacing={2} alignItems="start">
              <Grid item xs={12}>
                <TimeLineModeView
                  ...
                  columns={state?.columns}
                  onTaskClick={onTaskClick}
                  onCellClick={(event, cell) => onCellClick(cell.date)}
                  searchResult={searchResult}
